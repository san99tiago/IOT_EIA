//CODIGO RECEPTOR DE SERIAL CON OTRO PIC18F2550 ENVIANDO CARACTER "A"
...
//Se crea variable bandera para efectuar cambios en LEDs por info serial 
int bandera = 0;
...
void main(void) {
    ...
    //Se indican pines B3 y C2 como salidas (0 es output)
    TRISBbits.RB3 = 0;
    TRISCbits.RC2 = 0;
    
    //Se inicializan pines B3 y C2 en ON y OFF (respectivamente) al inicio
    LATBbits.LB3 = 1;
    LATCbits.LC2 = 0;
    ...
    while (1){
        //En caso de activar bandera desde interrupciones (por RX serial)
        if (bandera == 1){
            //Cambiamos estado de los LEDs de salida para ver recepcion
            LATBbits.LB3 = !LATBbits.LB3;  //Cambiamos estado pin B3
            LATCbits.LC2 = !LATCbits.LC2;  //Cambiamos estado pin C2
            __delay_ms(50);
            bandera = 0;  //Reseteamos bandera
        }
    } 
  return;
}
...
//DEFINIMOS LA FUNCION ASOCIADA A LAS INTERRUPCIONES (Recepcion Serial)
  void __interrupt() isr(){
      ...
      if(PIR1bits.RCIF){
        ...
        //Si caracter es 'A', entonces activamos bandera, de lo contrario NO
        if (recibido == 'A'){
            bandera = 1;
        }
        //Se baja bandera asociada a la interrupcion del serial RX
        PIR1bits.RCIF = 0;
      }
  }
