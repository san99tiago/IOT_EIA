  ...
  if (WiFi.status() == WL_CONNECTED) { 
    ...
    while(Serial.available() ) {
      ...
      if (INIT_PIC == 1){
        ...
        if (palabra_recibida == "DIA") {
          Serial.println("ILUMINACION:DIA");
          HACER_POST("DIA_NOCHE" , concatenar_json( DEVICE_ID , "DIA"));          
        }
        
        else if (palabra_recibida == "NOCHE") {
          Serial.println("ILUMINACION:NOCHE");
          HACER_POST("DIA_NOCHE" , concatenar_json( DEVICE_ID ,"NOCHE"));          
        }
...
String concatenar_json(String ID , String  light) {
  ...
  String JSON = "{\"device_id\":\"" + ID + "\",\"light\":\"" + light + "\"}";
